...
**Third, the agentic self-critique layer changed the eval result on the case that most surprised me during Phase 3.** On the pre-agentic run the "purple mathematics tuesday" nonsense query failed test 8: the LLM commentary quoted the nonsense back as if it were a song title, and Guardrail 3 correctly flagged it. On the post-agentic run the same input passes. I didn't retrain anything and I didn't change the retrieval, the scorer, or Guardrail 3. What changed is that between commentary generation and Guardrail 3, another Gemini call now reviews the draft and returns a structured verdict. Whether the improvement comes from the critic actively catching the issue and forcing a rewrite, or from v1 coming out cleaner because the model "knows" a critic will review it, is genuinely ambiguous from the outside. The trace tells me per query, but the aggregate improvement is real either way. I hadn't predicted that adding a review stage would improve the *initial* draft's quality, but that appears to be what's happening in some fraction of cases.

---

## 13. AI Collaboration During Development

I built P4 in an extended pairing session with Claude (Anthropic's Sonnet). The pattern was collaborative and iterative: I set the direction for each phase, made the architectural calls that shaped the pipeline, and reviewed every proposal against the actual output before it stayed in the repo. Claude was fast and useful for the mechanical work: drafting boilerplate, proposing test scaffolds, catching syntax I would have had to look up. But the substantive choices, what to build, what to cut, when to stop, what looked right and what didn't, stayed with me. Two examples make this concrete: I read every LLM output as it came out and rejected two grounding-regex fixes before we shipped the third; I insisted on real reproducibility evidence in the README rather than accepting a "trust me it works" summary; and when we hit ambiguity about time budget on the stretch goals, I chose which ones to attempt based on my own read of the risk. The most useful thing Claude did was propose *specific* shapes fast enough that I could evaluate and course-correct without losing momentum. The most useful thing I did was refuse to ship anything I hadn't checked with my own eyes.
...
**A second helpful AI suggestion.**
When we scoped the agentic workflow stretch late in the session, I was tempted by heavier patterns from Gemini's examples online: planner + tool-caller + verifier chains. Claude's counter-proposal was a critic-and-reviser loop wrapping the existing commentary step, with the critic returning structured JSON and the pipeline branching on that verdict. I evaluated the trade-off and agreed: a heavier planner would introduce new failure modes (tool-call parsing, planner-hallucinated steps) that we would need to test, and we didn't have time. Keeping the risk isolated to two extra LLM calls that the existing fallback already handles was the pragmatic choice. That decision paid off. The loop went from designed to committed in about 25 minutes and immediately turned the 7/8 eval into 8/8.

**A second flawed AI suggestion.**
When I asked to make the agentic layer visible in the smoke test output, Claude's first proposal was to print the critic's raw JSON response, the full v1 commentary, and the full v2 commentary side by side. I could see immediately that would produce a wall of text three screens tall per query and bury the actual recommendations. I rejected it and asked for the smallest signal that would prove the loop was firing, and we landed on the two-line summary now in the code (one `Self-critique:` line with the parsed verdict, one `Action:` line saying accepted or revised). The pattern I've noticed across the session: an AI collaborator tends to optimize for showing *everything*, when the actually useful move is to show the smallest thing that proves the pipeline is doing what you claimed. Recognizing that gap and pushing back on it is the human judgment call.
